class AppsController < ApplicationController
  ...
  def create_tasks_table
    ft_id = App.create_basic_tasks_table(current_user.email)
    render :json => {:ft_table_id => ft_id}.to_json
  end

  def create_answers_table
    ft_id = App.create_basic_answers_table(current_user.email)
    render :json => {:ft_table_id => ft_id}.to_json
  end
  ...
  def source
    @app = App.find(params[:id])
    if (current_or_guest_username != @app.user.username and !@app.gist_id.nil?)
...
# POST /apps.json
  def create
    ...
    respond_to do |format|
      ...
      if @app.save
         # default template if no gist
         @app.gist_id = GistDao.instance.fork_gists("3543287") if @app.gist_id.nil?
